...
#ifndef PRODUCT_NAME_LABEL
#define PRODUCT_NAME_LABEL OpenCensus
#endif
...
namespace {
  ...
  constexpr char kProductName[] = #PRODUCT_NAME_LABEL ;
...
/*
   ...
   FUNCTIONALITY TODO:
      ...
      - discriminate by tags ?
         - gce_instance instance_id numeric ( as stackdriver ) or hostname ? ( not part of data structure )
      ...
      - observe specific process names ? /proc/[pid]/status, /proc[pid]/smap_rollup ? /proc/[pid]/oom_* ?
        - I have an example to that effect that uses tags for pid, ppid, and executable name
...
int main( int argc, char** argv )
{
  ...
  absl::SetProgramUsageMessage("Simple example reading /proc/meminfo and eventually /proc/vmstat and moving them to stackdriver" );
  ...
  if ( absl::GetFlag( FLAGS_debug ) ) {
    ...
    statsOps.metric_name_prefix = absl::StrCat( "custom.googleapis.com/", kProductName );
    ...
    if ( instance_id.empty() ){
      ...
      // name does the hostname, what a human would like
        auto r = cpr::Get(cpr::Url{"http://169.254.169.254/computeMetadata/v1/instance/id"},
                          cpr::Header{{"Metadata-Flavor", "Google"}} );
      ...
      // name does the hostname, what a human would like
        ...
        if ( r.status_code == 200 ){
          ...
          if ( status == 0 ){
            ...
            std::cerr << "could not obtain host name using gethostname(2) ..." << std::endl;
            return 1;
        }
    }
    ...
    (*statsOps.monitored_resource.mutable_labels())[ kProjectIDLabel     ] = project_id;
    (*statsOps.monitored_resource.mutable_labels())[ kGCEInstanceIDLabel ] = instance_id; // instance_name; ?
  ...
  // use libre2 because CentOS 7 devtoolset packages a gcc 7+ which reuses the default GCC 4.8.x libstdc++
  // which has buggy / incomplete support for regex
  // as a result, unix-style regular expression are buggy...
  ...
  if ( ! ( doMemInfo || doVmStat ) ){
    std::cerr << "Nothing to do, please select --meminfo or --vmstat or both" << std::endl;
    return 0;
  }
  ...
  while ( true ){
    ...
    if ( firstTime ){
      ...
      for ( auto const & p : meminfoByteValues ){
        ...
        opencensus::stats::MeasureInt64 m( opencensus::stats::MeasureInt64::Register( mn, p.first, kBytesUnits ) );

        auto vn = absl::StrCat( "proc/meminfo/", p.first ); // do I want a suffix for aggregation / units ?
        ...
        auto vd = opencensus::stats::ViewDescriptor()
          .set_name( vn )
          .set_measure( mn )
          .set_aggregation( opencensus::stats::Aggregation::LastValue() ) // this is correct. instantaneous measure.
      ...
      for ( auto const & p : meminfoCountValues ){
        auto mn = absl::StrCat( "proc/meminfo/", p.first );
        opencensus::stats::MeasureInt64 m( opencensus::stats::MeasureInt64::Register( mn, p.first, kNoUnits ) );
      ...
      for ( auto const & p : vmstatCountValues ){
        ...
        auto vd = opencensus::stats::ViewDescriptor()
                  .set_name( vn )
                  .set_measure( mn )
                  .set_aggregation( opencensus::stats::Aggregation::LastValue() )  // FIXME: mostly _wrong_ some are cumulative ..., some may be deltas ?
                  .set_description( desc );
